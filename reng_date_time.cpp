...
// reng_date_time.cpp
// FUP Engine 
// reng_date and reng_time class implementation
...
bool reng_time::set_time(int h, int m, int s)
{
	...
	{
		fprintf(fp,"bad hour specifier %d\n",h);
		h=0;
		valid=false;
	}
	...
	{
		fprintf(fp,"bad minute specifier %d\n",m);
		m=0;
		valid=false;
	}
...
reng_time reng_time::operator+ (int s)  const
{
	...
	if (s >= 3600)
		fprintf(fp," time Wrap round \n");
	
	if (s < 0)
	{
		fprintf(fp," Cant decrement secs \n");
		exit(1);
	}
...
bool reng_time::set_h_m(const char *t)
{
	...
	if(strlen(t) != 4)
	{
		set_time(0,0,0);
		valid=false;
		fprintf(fp,"bad string to convert in set_h_m %s",t);
...
bool reng_time::set_time(char *t)
{
	...
	if(strlen(t) != 6)
	{
		set_time(0,0,0);
		valid=false;
		fprintf(fp,"bad string to convert in set_time %s",t);
		return valid;
	}
...
bool reng_date::set_date(char *dte)
{
	...
	if(strlen(dte) < 8)
	{
		fprintf(fp," Not enough characters for year conversion {%s} ",dte);
		set_date(1972,7,10);
		return false;
	}
...
bool reng_date::set_date(int y, int m, int d)
{
	...
	{
		fprintf(fp,"bad year specifier %d\n",y);
		y = MIN_YEAR;
		valid = false;
	}
	...
	{
		fprintf(fp,"bad month specifier %d\n",m);
		m = 1;
		valid = false;
	}
...
bool reng_date_time::set_date_time(char * a)
{
	...
	if( t.length() != 14)
	{
		fprintf(fp," ERROR Bad length in date time conversion\n");
		return false;
	}
